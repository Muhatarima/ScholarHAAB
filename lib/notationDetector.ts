...
const CHEMICAL_FORMULA_PATTERN =
  /(?:\b(?:[A-Z][a-z]?[\d₀-₉]*|[()]){2,}(?:[²³¹⁰⁴⁵⁶⁷⁸⁹⁺⁻]+)?(?=$|[\s,.;:)\]}+→⇌<=>-])|\b(?:mol|dm|kg|ms|m|s|N|J|V|A|Ω)(?:[⁻⁰¹²³⁴⁵⁶⁷⁸⁹-]+)?\b)/
const CHEMICAL_EQUATION_PATTERN =
  /(?:\b\d*)?(?:[A-Z][a-z]?[\d₀-₉]*|[()]){2,}(?:[²³¹⁰⁴⁵⁶⁷⁸⁹⁺⁻]+)?(?:\s*(?:\+|→|⇌|<=>|->)\s*(?:\d*)?(?:[A-Z][a-z]?[\d₀-₉]*|[()]){2,}(?:[²³¹⁰⁴⁵⁶⁷⁸⁹⁺⁻]+)?)+/u
const MATH_TOKEN_PATTERN =
  /(?:\b(?:d[²2]?y\/d[²2]?x[²2]?|dy\/dx|d2y\/dx2|∂[A-Za-z]\/∂[A-Za-z]|lim(?:_\{?[^}\s]+\}?)?|sin[²³⁻¹0-9]*[A-Za-zθπ]?|cos[²³⁻¹0-9]*[A-Za-zθπ]?|tan[²³⁻¹0-9]*[A-Za-zθπ]?|log[₀-₉0-9]*|ln)\b|[αβγδΔθΘλμπρσωΩ]|[A-Za-z]⃗|[A-Za-zθπλμ][₀-₉0-9]*[²³⁰¹⁴⁵⁶⁷⁸⁹⁻⁺^]?(?:\s*[+\-−=*/×÷]\s*[A-Za-z0-9θπλμ√∛][A-Za-z0-9₀-₉²³⁰¹⁴⁵⁶⁷⁸⁹⁻⁺^]*)+|[A-Za-zθπλμ][₀-₉0-9]*[²³⁰¹⁴⁵⁶⁷⁸⁹⁻⁺^]+|√\s*[A-Za-z0-9]+|∛\s*[A-Za-z0-9]+|∫[^\s,.;:)]*|∑[^\s,.;:)]*|\d+(?:\.\d+)?\s*(?:×|x)\s*10[⁻⁺²³⁰¹⁴⁵⁶⁷⁸⁹^+-]*|\d+(?:\.\d+)?\s*(?:m\/s²|ms⁻²|kgm⁻³|mol\s*dm⁻³|nm|kg|m|s|N|J|V|A)\b|[¼½¾⅓⅔⅛⅜⅝⅞]|[A-Za-z]\s*=\s*[A-Za-z0-9θπλμ√∛][^,.;\n]*)/u
